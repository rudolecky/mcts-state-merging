"""CLI: classical MCTS merging on Three Men's Morris (movement phase) -- no
LLM, no learned metric. Third no-LLM domain after Connect Four and the
8-puzzle, filling the adversarial+reversible cell. Mirrors those runners'
structure and reuses the generic bootstrap-CI helper.
"""
...
from .classical_mcts_morris import ClassicalMCTSConfig, is_solved, run_search
...
def _solve_rate(instance, merge_enabled: bool, budget: int, rollout_depth: int, repeats: int, base_seed: int) -> tuple[float, float]:
    ...
    for r in range(repeats):
        ...
        graph = run_search(board, instance.to_move, config, budget, rollout_depth, rng)
        if is_solved(graph):
            solved += 1
...
def main():
    ap = argparse.ArgumentParser(description="Classical (no-LLM) MCTS merge-vs-tree experiment on Three Men's Morris")
    ap.add_argument("--k-plies", type=int, nargs="+", default=[1, 3, 5])
    ap.add_argument("--n-puzzles", type=int, default=15, help="puzzles per k_plies value")
    ...
    ap.add_argument("--output-dir", default="results/classical_mcts_morris_experiment")
    args = ap.parse_args()
