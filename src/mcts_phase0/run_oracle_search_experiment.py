"""CLI: PUCT guided by a perfect ground-truth oracle value (search_oracle.py)
vs. run_random_search, matched budget, on ProsQA. Distinguishes the two
mechanisms left open in RANDOM_VS_MCTS_FINDINGS.md for why the *learned*
projection didn't prevent the persistent tree-vs-random gap: (1) the
learned signal was too weak/noisy, vs. (2) budget=15 is structurally too
thin for how PUCT allocates it, independent of guidance quality. If
oracle-guided tree still loses badly here, (2) is the real culprit; if it
closes the gap, (1) was. Mirrors run_random_search_experiment.py's infra
(same projection/tau/held-out set) -- the projection is still loaded here
only because expand_node_oracle still computes proj_value for the (unused,
merge_enabled=False) merge-target check, matching every other value_source
in search.py's own convention.
"""
...
from .search_oracle import run_search_oracle


def main():
    import argparse

    ap = argparse.ArgumentParser(description="Oracle-guided PUCT (no merge) vs. pure random search, matched budget, on ProsQA")
    ...
    ap.add_argument("--output-dir", default="results/oracle_search_experiment_qwen7b_prosqa")
    args = ap.parse_args()
    ...
    for inst in tqdm(held_out, desc="problems"):
        ...
        graph = run_search_oracle(lm, prompt_ids, prompt_len, inst, prosqa.parse_and_verify, config, args.budget)
        tree_elapsed = time.time() - t0
        per_problem["tree_oracle"] = {"solved": puct_is_solved(graph), "elapsed_sec": tree_elapsed}
        ...
        print(f"  {inst.id}: tree_oracle={per_problem['tree_oracle']['solved']} ({tree_elapsed:.1f}s) "
              f"random={per_problem['random']['solved']} ({random_elapsed:.1f}s)")

    elapsed_total = time.time() - start
    out_dir = Path(args.output_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / f"oracle_search_results_seed{args.held_out_seed}.json"
    with open(out_path, "w") as f:
        json.dump({"args": vars(args), "elapsed_sec": elapsed_total, "results": results}, f, indent=2)

    for arm in ["tree_oracle", "random"]:
