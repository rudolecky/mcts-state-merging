"""CLI: classical MCTS merging on the sliding-tile (8-)puzzle -- no LLM, no
learned metric. Second no-LLM domain after Connect Four
(run_classical_mcts_experiment.py), chosen to raise transposition density.
Mirrors that runner's structure and reuses its generic bootstrap-CI helper.
"""
...
from .classical_mcts_puzzle import ClassicalMCTSConfig, is_solved, run_search
from .datasets import sliding_puzzle_engine
...
def _solve_rate(instance, merge_enabled: bool, budget: int, rollout_depth: int, repeats: int, base_seed: int) -> tuple[float, float]:
    ...
    for r in range(repeats):
        ...
        graph = run_search(instance.start_state, instance.width, instance.height, config, budget, rollout_depth, rng)
        if is_solved(graph):
            solved += 1
...
def main():
    ap = argparse.ArgumentParser(description="Classical (no-LLM) MCTS merge-vs-tree experiment on the sliding-tile puzzle")
    ap.add_argument("--width", type=int, default=3)
    ap.add_argument("--height", type=int, default=3)
    ap.add_argument("--target-distances", type=int, nargs="+", default=[4, 8, 12])
    ...
    ap.add_argument("--output-dir", default="results/classical_mcts_puzzle_experiment")
    args = ap.parse_args()

    start = time.time()
    distances = sliding_puzzle_engine.bfs_distances(args.width, args.height)
    print(f"computed BFS distances for {len(distances)} solvable states in {time.time() - start:.2f}s")

    puzzles = []
    for td in args.target_distances:
        puzzles.extend(sliding_puzzle_engine.generate_puzzles(
            n=args.n_puzzles, seed=args.seed, width=args.width, height=args.height,
            target_distance=td, distances=distances,
        ))
